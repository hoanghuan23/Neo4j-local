...
def extract_knowledge(content: str, call_model=None) -> dict:
    ...
    Với địa danh hành chính Việt Nam được gọi bằng “tỉnh”, “thành phố” hoặc “TP.”, `name` giữ cách gọi trực tiếp trong văn bản nhưng `canonical_name` phải chỉ là tên riêng và bỏ tiền tố đó. Ví dụ “tỉnh Bắc Ninh”, “thành phố Bắc Ninh” hoặc “TP. Bắc Ninh” đều dùng `canonical_name="Bắc Ninh"`; tuyệt đối không tạo canonical_name “tỉnh Bắc Ninh” hay “thành phố Bắc Ninh”. Nếu cùng tên riêng xuất hiện với các tiền tố này hoặc cả dạng trần, coi là một LOCATION và chỉ trả một Entity, ưu tiên tái sử dụng tên trần làm canonical_name.
