...
MODEL_PRICING = {
    "gpt-5.6-luna": ModelPricing(Decimal("0.20"), Decimal("1.20")),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Test một model OpenAI trên nhiều Post bằng prompt, schema và "
    ...
    parser.add_argument(
        ...
        help=f"Tên model OpenAI (mặc định: {DEFAULT_MODEL})",
    )
...
def token_usage_from_response(response: Any) -> TokenUsage:
    ...
    if usage is None:
        raise ValueError("OpenAI không trả về usage; không thể tính chi phí")
...
class OpenAICaller:
    ...
    def __call__(self, prompt: str, output_schema: dict) -> dict:
        ...
        try:
            ...
            raise ValueError("OpenAI trả về nội dung không phải JSON hợp lệ") from error
...
def main() -> None:
    ...
    if not api_key:
        raise SystemExit("Chưa cấu hình OPENAI_API_KEY trong .env")
    ...
    call_openai = OpenAICaller(client, args.model)
    ...
    try:
        for index, record in enumerate(records, start=1):
            ...
            classifier_usage = call_openai.last_usage
            ...
            if needs_deep:
                deep_count += 1
                raw_result = extract_knowledge(content, call_model=call_openai)
                deep_usage = call_openai.last_usage
